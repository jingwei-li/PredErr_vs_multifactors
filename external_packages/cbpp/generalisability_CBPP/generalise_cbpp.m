...
if(strcmp(dataset, 'HCP-A_fluidcog') || strcmp(dataset, 'HCP-A_openness'))
    load(fullfile(in_dir, ['HCPA_fix_resid0_' atlas '_Pearson.mat']), 'fc', 'y', 'conf');
else
...
case {'HCP-A_fluidcog', 'HCP-A_openness'}
    % Openness measure has the longest subject list --added by Jingwei
    sublist_full = fullfile(fileparts(script_dir), 'bin', 'sublist', ['HCP-A_openness_allRun_sub.csv']);
    if nargin < 7
        sublist = fullfile(fileparts(script_dir), 'bin', 'sublist', [dataset '_allRun_sub.csv']);
    end
    % -- Added by Jingwei
    [~, ~, idx] = intersect(CBIG_text2cell(sublist), CBIG_text2cell(sublist_full), 'stable');
    switch dataset
    case 'HCP-A_fluidcog'
        y = y.CogFluidComp_AgeAdj(idx);
    case 'HCP-A_openness'
        y = y.NEOFAC_O(idx);
    otherwise
