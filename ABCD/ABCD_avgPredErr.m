function ABCD_avgPredErr(model_dir, full_bhvr_ls, bhvr_cls_csv, PredErr_in, PredErr_out)

% ABCD_avg_PredErr(model_dir, full_bhvr_ls, bhvr_cls_csv, PredErr_in, PredErr_out)
%
% 
...
for c = 1:length(headers)
    ...
    for b = 1:length(bhvr_cls)
        ...
        err_norm.(headers{c}) = [err_norm.(headers{c}) all_err.err(:,idx) ./ std(y, 1)];
    end
