function extract_rest_timeseries(sub_data_csv)
    ...
    out_proj_dir = '/data/project/predict_stereotype';
    outdir = fullfile(out_proj_dir, 'results', 'ABCD', 'parcellated_timeseries');
    sub_data = readtable(sub_data_csv);
    load(fullfile(out_proj_dir, 'results', 'ABCD', 'censor', 'ABCD_censor.mat'));
    ...
    for i = 1:size(sub_data, 1)
        ...
        system(sprintf('git -C %s config --local --add remote.datalad.annex-ignore false', s));
        cd(fullfile(indir, s, ses, 'func'));

        for run = runs
            out_file = fullfile(outdir, [s '_' ses '_' char(run) '_S4_timeseries.mat']);
            if isfile(out_file)
                fprintf("Output for %s %s already exists.\n", s, char(run));
            else
                ...
                ts_fslr_sm.dtseries = ts_fslr_sm.dtseries(1:size(parc_sch.dlabel, 1), :);
                % Set regions with nans to zero
                ts_fslr_sm.dtseries(isnan(ts_fslr_sm.dtseries)) = 0;
                ...
                [ts_fslr_cut, ts_mni_cut, melodic_cut, nss_vol] = rm_nonsteady(ts_fslr_sm, ts_mni, melodic);
                ts_fslr_aroma = fslr_apply_aroma(ts_fslr_cut, melodic_cut, filter);

                mt_tsv = [s '_' ses '_task-rest_' char(run) '_desc-confounds_timeseries.tsv'];
                system(sprintf('datalad get %s', mt_tsv));
                mt_conf = readtable(mt_tsv, "FileType", "text", "Delimiter", "\t");
                [ts_fslr_resid, ts_mni_resid] = nuisance_reg(ts_fslr_aroma, ts_mni_cut, mt_conf, nss_vol);

                disp(size(ts_fslr_resid))
                disp(size(ts_mni_resid))
                disp(size(parc_sch.dlabel))

                pts = zeros(454, size(ts_fslr_resid, 1));
                for roi = 1:400
                    pts(roi, :) = mean(ts_fslr_resid(:, parc_sch.dlabel==roi), 2);
                end
                for roi = 1:54
                    pts(roi+400, :) = mean(ts_mni_resid(:, find(parc_mel.vol == roi)), 2);
                end

                save(out_file, 'pts', '-v7.3');
...
function [ts_fslr, ts_mni, melodic, nonss_vol] = rm_nonsteady(ts_fslr, ts_mni, melodic)
...
function ts_fslr_aroma = fslr_apply_aroma(ts_fslr, melodic, filter)
...
function [ts_fslr_resid, ts_mni_resid] = nuisance_reg(ts_surf, ts_vol, mt_conf, nonss_vol)
    ...
    regressors = [mt_conf.global_signal, mt_conf.global_signal_derivative1, ...
                  ...
                  mt_conf.white_matter, mt_conf.white_matter_derivative1, ...
                  ...
                  mt_conf.trans_z, mt_conf.trans_z_derivative1, ...
                  mt_conf.trans_z_power2, mt_conf.trans_z_derivative1_power2];
    regressors(nonss_vol, :) = [];

    [ts_fslr_resid, ~, ~, ~] = CBIG_glm_regress_matrix(single(ts_surf)', regressors, 1, []);
    [ts_mni_resid, ~, ~, ~] = CBIG_glm_regress_matrix(single(ts_vol.vol)', regressors, 1, []);
end
