function ABCD_behavior_vs_sociodemograph(subj_ls, avgBehavior, outdir, bhvr_cls_names, metric)

% ABCD_behavior_vs_sociodemograph(subj_ls, avgBehavior, outdir, bhvr_cls_names, metric)
%
% Inputs:
%   - subj_ls
...
case 'handedness'
    ...
    ABCD_violin_PredErr_vs_other_var(handedness, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
case 'prt_educ'
    ...
    Ylabel = 'Behavioral scores';
    outbase = 'behavior_vs_Prt1_Educ';
    ABCD_violin_PredErr_vs_other_var(peduc(:,1), bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)

    err_names = fieldnames(bhvr_avg);
    for s = 1:length(uq_st)
        site_idx = strcmp(site, uq_st{s});
        if(length(find(site_idx==1)) > 300)
            new_err = bhvr_avg;
            for c = 1:length(err_names);
                new_err.(err_names{c}) = bhvr_avg.(err_names{c})(site_idx);
            end
            outbase = ['behavior_vs_Prt1_Educ_' uq_st{s}];
    ...
    outbase = 'behavior_vs_Prt1_Educ_BioMother';
    ABCD_violin_PredErr_vs_other_var(peduc_bm, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)

    % plot parent 2's education
    Xlabel = peduc_colloquial{2};
    Ylabel = 'Behavioral scores';
    outbase = 'behavior_vs_Prt2_Educ';
    ABCD_violin_PredErr_vs_other_var(peduc(:,2), bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
case 'income'
    ...
    ABCD_violin_PredErr_vs_other_var(income, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
    ...
    for s = 1:length(uq_st)
        ...
        if(length(find(site_idx==1)) > 300)
            ...
            outbase = ['behavior_vs_income_' uq_st{s}];
...
case 'address_size'
    ...
    Ylabel = 'Behavioral scores';
    outbase = 'behavior_vs_AddrSize';
    ABCD_violin_PredErr_vs_other_var(add_size, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
case 'ethnicity'
    ...
    ABCD_violin_PredErr_vs_other_var(ethnicity, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
    ...
    for s = 1:length(uq_st)
        ...
        if(length(find(site_idx==1)) > 300)
            ...
            outbase = ['behavior_vs_ethnicity_' uq_st{s}];
...
case 'age'
    ...
    Ylabel = 'Age';
    outbase = 'behavior_vs_age_scatter';
    ABCD_scatter_PredErr_vs_other_var(bhvr_avg, age, outdir, outbase, bhvr_cls_names, Ylabel, 1)

    XTickLabels = arrayfun(@num2str, unique(age(~isnan(age))), 'UniformOutput', 0);
    Xlabel = 'Age';
    Ylabel = 'Behavioral scores';
    outbase = 'behavior_vs_age';
    ABCD_violin_PredErr_vs_other_var(age, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
    ...
    Xlabel = 'Age';
    outbase = 'behavior_vs_age_siteReg_scatter';
    ABCD_scatter_PredErr_vs_other_var(bhvr_avg, resid, outdir, outbase, bhvr_cls_names, Ylabel, 1)
case 'sex'
    ...
    Xlabel = 'Sex';
    Ylabel = 'Behavioral scores';
    outbase = 'behavior_vs_sex';
    ABCD_violin_PredErr_vs_other_var(sex, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
    ...
    for s = 1:length(uq_st)
        ...
        if(length(find(site_idx==1)) > 300)
            ...
            outbase = ['behavior_vs_sex_' uq_st{s}];
...
case 'site'
    ...
    Xlabel = 'Site';
    Ylabel = 'Behavioral scores';
    outbase = 'behavior_vs_site';
    ABCD_violin_PredErr_vs_other_var(Xdata, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
    ...
    outbase = 'behavior_vs_site_removeSingleDataPoint';
    ABCD_violin_PredErr_vs_other_var(Xdata2, bhvr_avg, outdir, outbase, XTickLabels, Xlabel, Ylabel, bhvr_cls_names, 1)
otherwise
