...
data = pd.read_csv(args.sub_list, header=None, names=['subjectkey'])
data["Sub_Key"] = data["subjectkey"].str.removesuffix("_V1_MR")
...
conf_list_new = ['interview_age', 'FD', 'DVARS', 'ICV']
conf_list_drop = []
conf_list_pred = ['interview_age', 'sex', 'education', 'FD', 'DVARS', 'ICV']
...
for c in new_data.columns:
    ...
    conf_list_drop.append(conf_list[1]+'_'+c)
...
for c in new_data.columns:
    ...
    conf_list_pred.append(conf_list[2]+'_'+c)
...
for c in new_data.columns:
    ...
    conf_list_drop.append(conf_list[3]+'_'+c.replace(" ", "_").replace(',',''))
...
data = data.merge(subjects, how='inner', on='subjectkey')
data_split = data.copy()
data_split.drop(columns=['sex', 'education'], inplace=True)
data.drop(columns=conf_list_drop, inplace=True)

# save outputs separately
data[['subjectkey']+colnames].to_csv((args.out_dir + '/HCP-D_y.csv'), index=None)
data[['subjectkey']+conf_list_pred].to_csv((args.out_dir + '/HCP-D_conf.csv'), index=None)
data_split[['subjectkey']+conf_list_new].to_csv(
    (args.out_dir + '/HCP-D_conf_split.csv'), index=None)
data[['subjectkey']].to_csv(args.out_dir + '/sublist_allbehavior.csv', index=None, header=None)
with open(os.path.join(args.out_dir, 'confound_list.txt'), 'w') as f:
    for item in conf_list_new:
        f.write("%s\n" % item)

print(data[['subjectkey']+conf_list_pred])
